...
function LIneLight({ size = "200px", Addclass }) {
  return (
    <p className={`lineLight
      animate__animated 
      animate__delay-2s 
 animate__slower
    ${Addclass}
    `} style={{ width: size, background: colorAsl }}></p>
  )
}

function IconLogo({ children }) {
  return (
    <div className={`icon-logo animate__animated
    animate__fadeIn 
    animate__delay-1s 
  ...
  return (
    <div className={`portfolio-card `}>
      ...
      <div className="portfolio-card-content">
        ...
        {link && (
          <a href={link} target="_blank" rel="noopener noreferrer" style={{ background: colorAsl }}>
            View Project
          </a>
        )}
      </div>
...
function Introduction({
  ...
  useEffect(() => {
    initParticlesEngine(async (engine) => {
      await loadSlim(engine); // نسخه سبک tsparticles
    }).then(() => setInit(true));
  }, []);
  ...
  return (
    ...
    animate__delay-70ms 
    animate__slower
    " style={{ color: "#ffff", background: colorAsl, }}  >
            Download pdf
          </button>
